...
export interface Researcher {
   ...
   *  Per Pravilnik IER v2.2 (Priloga 3, Pogoj 2): cumulative value of
   *  projects outside ARIS where candidate had a leading role
   *  (vodja projekta / vodilna vloga), expressed in FTE units
   *  (znesek sredstev / cena FTE kategorije A). */
...
export interface LeadershipRecord {
  /** Cumulative FTE value of led/coordinated projects or work packages.
   *  Per Pravilnik IER v2.2, Priloga 3, Pogoj 3, paths (a)–(d): sum of
   *  total project values (for the Institute AND other partners) on
   *  projects where the candidate had a leading role at PROJECT level
   *  (not only at Institute level), expressed in FTE units of category A
   *  in the year the project was awarded. */
  cumulativeValueFte: number;
